...
def create(db_path, overwrite=False):
	...
	if ((not cursor.fetchone()) or overwrite == True):
		...
		conn.execute("INSERT INTO SEARCH (ID, NAME, COMMENT, DATECREATED, SECRET, DISPLAY) \
					VALUES (4, 'Marvin4', 'I''ve calculated your chance of survival, but I don''t think you''ll like it.', '2015-08-04 12:00:00', 'Okifyoustealme', 0)")
		conn.execute("INSERT INTO SEARCH (ID, NAME, COMMENT, DATECREATED, SECRET, DISPLAY) \
...
def search(search):
	conn = sqlite3.connect(sqli_db_path)
	cursor = conn.execute("SELECT ID, NAME, COMMENT, DATECREATED FROM SEARCH WHERE DISPLAY <> 0 AND NAME LIKE '%" + search + "%' ORDER BY ID")
