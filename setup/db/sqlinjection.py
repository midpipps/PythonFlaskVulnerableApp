...
def create(db_path, overwrite=False):
    ...
    if ((not cursor.fetchone()) or overwrite == True):
        ...
        search_insert('Marvin1', '555-555-5555', 'Sureasecret', 1, "'2016-01-01 12:00:00'", conn)
        search_insert('Marvin2', '555-777-7777', 'Thisisnotassecretasyouthink', 1, "'2015-07-23 12:00:00'", conn)
        search_insert('Marvin3', '555-555-1212', 'OhNoesMySecrets', 1, "'2016-11-12 12:00:00'", conn)
        search_insert('Marvin4', '666-555-5555', 'Okifyoustealme', 0, "'2015-08-04 12:00:00'", conn)
        search_insert('Marvin5', '555-999-5555', 'NotSoSecret', 1, "'2018-09-20 12:00:00'", conn)
        search_insert('SecretUser', '424-242-4242', '44af595c-1b56-400d-951e-8407249c8446', 0, "'2014-08-16 12:00:00'", conn)
        conn.commit()
    conn.close()
    logging.info('SimpleInjection should be good to go')

def search_insert(name, phone, secret, display = 1, dt = "DATETIME('now')", conn=None):
    sql_string = "INSERT INTO SEARCH (NAME, PHONE, DATECREATED, SECRET, DISPLAY) VALUES ('" + name + "', '" + phone + "', " +  dt + ", '" + secret + "', " + str(display) + ")"
    if (conn):
